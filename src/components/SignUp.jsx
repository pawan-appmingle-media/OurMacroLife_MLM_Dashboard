...
const SignUp = () => {
  ...
  const resendOTP = async (phone_number) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/vendor/resend-otp`,
        { phone_number },
  ...
  const signupUser = async (signupData) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/vendor/signup`,
        signupData,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
      ...
      alert(error.response.data.error.phone_number[0] || "An error occurred");
    }
  };
  ...
  const verifyOtp = async (otp) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/vendor/verify-otp`,
        { otp, phone_number }
      );
